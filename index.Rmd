...
When you come across errors in your code (and you will) the first thing to do is to check carefully what you have submitted. Error messages in R can be a little bit cryptic to decipher, but they always contain some something which should lead you towards what the problem is. But it definitely requires a little bit of practice, detective work and trial and error to help negotiate. The first things to do are to check carefully for any clues in the error message - it might help point to the specific part of the code where you made a mistake.
...
Similarly we cannot use any object until after we have created it - the order in which we run code matters. For example:
...
*QUESTION* Try updating the code below to solving the theorem for the hypotenuse of a triangle with two sides of length of 10 and 22
